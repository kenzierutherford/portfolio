...
function sendPlasticMessage_A(sender) {
    let messageData = { text:"You chose 'tub, bottle or cup'. Please type if your item is 'motoroil or antifreeze', 'prescription vial' or 'other' (such as a yogurt or shampoo container) " }
    request({
        url: 'https://graph.facebook.com/v2.6/me/messages',
        qs: {access_token:token},
        method: 'POST',
...
app.post('/webhook/', function (req, res) {
    ...
    for (let i = 0; i < messaging_events.length; i++) {
        ...
        if (event.message && event.message.text) {
            ...
            if (text.toLowerCase() =='prescription vial'){
                sendTrashMessage(sender)
                continue
            }
